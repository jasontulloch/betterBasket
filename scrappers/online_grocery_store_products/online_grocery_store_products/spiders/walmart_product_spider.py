...
class WalmartProductSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        json_file_path = os.path.join(base_path, 'urls', 'walmart_urls.json')
    ...
    def parse(self, response):
        ...
        while True:
            ...
            location_elements = my_response.xpath(
                '//header/section/div/div/div/button/div/div/div/div/div[2]/div/span/text()'
            ).getall()

            # Assuming there's only one city and one zip code
            if len(location_elements) > 0:
                location_text = location_elements[0].strip()
                if ',' in location_text:  # Check if there's a comma to split city and zip code
                    city, zipCode = location_text.split(',', 1)
                    city = city.strip()
                    zipCode = zipCode.strip()
                    print(f"City: {city}")
                    print(f"Zip Code: {zipCode}")
                else:
                    print("Location format is not as expected.")
            else:
                print("Location information not found or more than one location element found.")
                print(location_elements)

            # Find all pagination elements that might contain page numbers
            pagination_elements = my_response.xpath(
                '//nav[@aria-label="pagination"]//ul/li/a[@data-automation-id="page-number"]/text() | //nav[@aria-label="pagination"]//ul/li/div/text()'
            ).getall()
            ...
            # Get the last page number (which is the maximum number found)
            if page_numbers:
                last_page_number = max(page_numbers)
                self.log(f"The last page number is: {last_page_number}")
            else:
                self.log("No page numbers found.")
            ...
            for item in food_items:
                ...
                if item.xpath('.//div[2]/span/span/text()').get() is not None:
                    food_item['label'] = item.xpath('.//div[2]/span/span/text()').get()
                else:
            ...
            if self.next_page_no <= last_page_number:
